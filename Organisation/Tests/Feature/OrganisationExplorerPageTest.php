...
test('the explorer route requires the structure view capability', function (): void {
    ...
    // In-tenant but ungranted: resolves the tenant like any member, then hits
    // the capability gate. (A tenantless user never reaches the gate — the
    // platform resolves them out of every tenant first.)
    $stranger = explorerUser((int) $fixture['company']->id);
...
test('an hod sees only the assigned departments', function (): void {
    ...
    explorerGrant($hodUser, 'people.organisation.hod.view');
...
test('an employee sees the permitted directory only', function (): void {
    ...
    explorerGrant($user, 'people.organisation.employee.view');
