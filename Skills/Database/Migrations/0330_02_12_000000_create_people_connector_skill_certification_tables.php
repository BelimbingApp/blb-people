...
return new class extends Migration
{
    ...
    {
        Schema::table('people_connector_skill_skills', function (Blueprint $table): void {
            $table->unique(['id', 'tenant_id', 'company_entity_id'], 'pcs_skill_id_tenant_company_uq');
        });
        ...
        Schema::create('people_connector_skill_certifications', function (Blueprint $table): void {
            ...
            $table->unique(['id', 'tenant_id', 'company_entity_id'], 'pcs_cert_id_tenant_company_uq');
            $table->unique(['tenant_id', 'company_entity_id', 'external_reference'], 'pcs_cert_reference_uq');
            $table->unique(
                ['tenant_id', 'company_entity_id', 'supersedes_certification_id'],
                'pcs_cert_predecessor_uq',
            );
            ...
            $table->foreign(['company_entity_id', 'tenant_id'], 'pcs_cert_company_fk')
            ...
            $table->foreign(
                ['supersedes_certification_id', 'tenant_id', 'company_entity_id'],
                'pcs_cert_supersedes_fk',
            )
        ...
        Schema::create('people_connector_skill_certification_skills', function (Blueprint $table): void {
            ...
            $table->unique(['id', 'tenant_id', 'company_entity_id'], 'pcs_cert_skill_id_tenant_company_uq');
            ...
            $table->foreign('tenant_id', 'pcs_cert_skill_tenant_fk')
                ->references('id')->on('tenants')->restrictOnDelete();
            $table->foreign(['company_entity_id', 'tenant_id'], 'pcs_cert_skill_company_fk')
                ->references(['id', 'tenant_id'])
                ->on('companies')
                ->restrictOnDelete();
            $table->foreign(['certification_id', 'tenant_id', 'company_entity_id'], 'pcs_cert_skill_cert_fk')
                ->references(['id', 'tenant_id', 'company_entity_id'])
                ->on('people_connector_skill_certifications')
                ->restrictOnDelete();
            $table->foreign(['skill_id', 'tenant_id', 'company_entity_id'], 'pcs_cert_skill_skill_fk')
                ->references(['id', 'tenant_id', 'company_entity_id'])
    ...
    public function down(): void
    {
        ...
        Schema::table('people_connector_skill_skills', function (Blueprint $table): void {
            $table->dropUnique('pcs_skill_id_tenant_company_uq');
        });
    }
    ...
    private function createImmutabilityGuards(): void
    {
        ...
        if ($driver === 'pgsql') {
            DB::unprepared(<<<'SQL'
                CREATE OR REPLACE FUNCTION pcs_skill_certification_company_guard() RETURNS trigger AS $$
                BEGIN
                    IF NOT EXISTS (
                        SELECT 1
                        FROM employees
                        WHERE id = NEW.employee_entity_id
                            AND company_id = NEW.company_entity_id
                    ) THEN
                        RAISE EXCEPTION 'employee % does not belong to company %', NEW.employee_entity_id, NEW.company_entity_id;
                    END IF;
                    RETURN NEW;
                END;
                $$ LANGUAGE plpgsql;
                DROP TRIGGER IF EXISTS pcs_skill_certification_company_guard ON people_connector_skill_certifications;
                CREATE TRIGGER pcs_skill_certification_company_guard
                    BEFORE INSERT ON people_connector_skill_certifications
                    FOR EACH ROW EXECUTE FUNCTION pcs_skill_certification_company_guard();
            ...
            DB::unprepared(<<<'SQL'
                DROP TRIGGER IF EXISTS pcs_skill_certification_company_guard;
                ...
                WHEN NOT EXISTS (
                    ...
                    WHERE id = NEW.employee_entity_id AND company_id = NEW.company_entity_id
                )
                BEGIN SELECT RAISE(ABORT, 'employee does not belong to certification company'); END;
