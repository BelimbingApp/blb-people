...
test('the page and the export refuse a user without the register capability', function (): void {
    $f = skillRegFixture();
    $exports = static fn (): int => AuditAction::query()->where('event', Index::EXPORT_EVENT)->count();
    $beforeExports = $exports();
    ...
    // Guard: the export action answers through the same view capability on
    // its own request: mount clean as HR, then continue as a user without
    // the grant and begin the next request's authorization graph.
    ...
    $page->call('export')->assertForbidden();

    // Guard: a refused export records no export action (the platform's own
    // denial trail is not this page's audit row).
    skillRegFlushAudit();
    expect($exports())->toBe($beforeExports);
});
...
test('rows from the sibling company and from another tenant never appear', function (): void {
    ...
    $otherCatalog = app(SkillCatalogStore::class);
    $otherCategory = $otherCatalog->defineCategory((int) $otherCompany->id, 'reg-other', 'Other');
    $otherSkill = $otherCatalog->defineSkill((int) $otherCompany->id, new SkillDraft('reg-other.a', 'Other skill', 'Out of scope.', (int) $otherCategory->id));
    ...
    DB::table('people_connector_skill_assessments')->insert([
        ...
        'cycle' => 'annual', 'status' => 'draft', 'hod_verification' => 'pending',
...
test('the expiring-within filter boundary is date-safe', function (): void {
    ...
    // Guard: as-of + 31 days does not. viewData reads the last render, so
    // rows written outside Livewire need a re-render before asserting.
    $page->call('$refresh');
    expect($page->viewData('rows')->pluck('skill')->all())->toBe(['Register skill B']);

    // Guard: a time part on the boundary date still counts as that date.
    // (Submitted facts are immutable, so the time part arrives on a new row,
    // which is also the newest and therefore current.)
    skillRegAssessment($f, $f['e1'], $f['skillB'], AssessmentResultBand::Meets, ['valid_until' => '2026-10-07 15:00:00']);
    $page->call('$refresh');
