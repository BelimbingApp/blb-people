            ...
            @if ($canManage && $gaps->isNotEmpty())
                <x-ui.card><div class="grid gap-3 md:grid-cols-2">
                    <x-ui.select id="development-action-type" :label="__('Action type')" wire:model="actionType" required>
                            @foreach (\App\Domains\People\Skills\Enums\DevelopmentActionType::cases() as $type)
                    ...
                    <x-ui.card wire:key="action-{{ $action->id }}"><article class="space-y-2">
                        ...
                        @if ($canManage)
                            <div class="flex flex-wrap gap-2">
                                @if ($action->status === \App\Domains\People\Skills\Enums\DevelopmentActionStatus::Proposed)<x-ui.button wire:click="tailor({{ $action->id }})">{{ __('Apply form to proposal') }}</x-ui.button><x-ui.button wire:click="approve({{ $action->id }})">{{ __('Approve') }}</x-ui.button>@endif
                                @if (in_array($action->status, [\App\Domains\People\Skills\Enums\DevelopmentActionStatus::NotStarted, \App\Domains\People\Skills\Enums\DevelopmentActionStatus::Scheduled, \App\Domains\People\Skills\Enums\DevelopmentActionStatus::OnHold], true))<x-ui.button wire:click="start({{ $action->id }})">{{ __('Start') }}</x-ui.button>@endif
                            </div>
                            @if ($action->status->isOpen() && $action->status !== \App\Domains\People\Skills\Enums\DevelopmentActionStatus::PendingReassessment)
                            ...
                            @endif
                            @if ($action->status === \App\Domains\People\Skills\Enums\DevelopmentActionStatus::PendingReassessment)
