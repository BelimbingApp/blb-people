...
return new class extends Migration
{
    ...
    /**
     * Workforce subjects are native People records after the relocation, so
     * these point at the tables that own them rather than at the connector's
     * entity registry, which a People-only install never creates. Same rule R2
     * applied to the Skills tables: the company axis carries the tenant with
     * it, departments are reference entries, everything else is an employee.
     */
    private function entityReference(Blueprint $table, string $column, string $name): void
    {
        ...
        $table->foreign($column, $name)
            ->references('id')
            ->on($column === 'target_department_entity_id' ? 'people_reference_entries' : 'employees')
            ->restrictOnDelete();
    }
        ...
        if (DB::connection()->getDriverName() === 'pgsql') {
            DB::unprepared(<<<'SQL'
                ...
                BEGIN
                    ...
                    -- People has no entity merges, so nothing can legitimately
                    -- rewrite an audit row and the exception is gone rather than
                    -- widened. Dropping only the merge test would have permitted
                    -- any company_entity_id change.
