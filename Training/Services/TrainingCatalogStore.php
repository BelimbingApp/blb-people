...
class TrainingCatalogStore
{
    ...
    /**
     * The seam answers what the connector's WorkforceEntity table used to: is
     * this stable id an entity of this type that the current tenant can see?
     * It resolves through the provider rather than through a projection table,
     * so the connector's copy is not needed to validate a reference.
     */
    private function assertEntity(int $tenantId, int $companyEntityId, int $entityId, WorkforceResourceType $type, string $field): void
    {
