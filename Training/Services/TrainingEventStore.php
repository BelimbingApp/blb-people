...
final class TrainingEventStore
{
    ...
    private function validateDraft(int $companyEntityId, TrainingEventDraft $draft): TrainingCourse
    {
        ...
        if ($draft->targetDepartmentEntityId !== null && ! $this->department($companyEntityId, $draft->targetDepartmentEntityId)) {
    ...
    /**
     * A department with nobody in it is still a department, which is why this
     * asks the seam for the company's units rather than inferring them from
     * employees: a training event is most often scheduled for a team that is
     * being stood up.
     */
    private function department(int $companyEntityId, int $departmentEntityId): bool
    {
        foreach ($this->workforce->organizationUnits($companyEntityId) as $unit) {
            if ($unit->reference->externalId === (string) $departmentEntityId) {
                return true;
            }
        }
