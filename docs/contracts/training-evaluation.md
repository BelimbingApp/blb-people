...
Training provides the participant-evaluation model and visibility reader. Criteria version `0012-a.v1` also provides an authenticated employee Livewire form for five fixed ratings and one comment on the employee's own attended event, with update-in-place behavior until the 14-day window closes. HR assisted (paper) entry for a named participant is delivered under `people.training.evaluation.assign`: the row keeps the participant as employee subject, names the HR user as `submitted_by_user_id`, is marked `entry_source = assisted_paper` (a database guard refuses that source without an entering actor), keeps the paper reference in `notes`, follows the same 14-day window, and never replaces a completed evaluation. HR follow-up, aggregate reporting, broader criteria versions and correction/reopening after closure remain undelivered.
