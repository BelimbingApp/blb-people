...
Training provides the participant-evaluation model and visibility reader. Criteria version `0012-a.v1` also provides an authenticated employee Livewire form for five fixed ratings and one comment on the employee's own attended event, with update-in-place behavior until the 14-day window closes. Criteria version `0012-g.v1` adds the remaining ratings and free-text answers with a draft state and mandatory-question completion (issue #360). Assisted entry, HR follow-up, aggregate reporting and correction/reopening after closure remain undelivered.
