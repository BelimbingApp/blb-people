...
Projection completeness is defined by the declared resource vocabulary, not by the presence of an envelope field. A resource named in `supported_resources` must be projected with every required identity and display field; if a native record cannot satisfy that contract, projection fails closed with a domain exception that identifies the offending record. Optional relationships on an otherwise valid record may remain `null` where the wire schema declares them optional. A resource that is not implemented is omitted from `supported_resources`; its empty collection in the versioned envelope is therefore an explicit unsupported capability, not a claim that the provider authoritatively contains zero records. When position projection is added, its stable identity and required fields must follow the same fail-closed rule before `position` is advertised.
